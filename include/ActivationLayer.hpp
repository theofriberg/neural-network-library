...
public:
  ActivationLayer(std::function<Eigen::MatrixXd(Eigen::MatrixXd)> act_fn,
                  std::function<Eigen::MatrixXd(Eigen::MatrixXd)> act_prime,
                  std::string name)
      : Layer(name), _activation_fn(act_fn), _activation_prime(act_prime) {}

  /**
   * @brief Performs the forward pass of the activation layer.
   *
   * This function applies the activation function to the input matrix and
   * caches the input for use in the backward pass.
   *
   ...
   *
   * This function computes the gradients of the input with respect to the given
   * gradient, and applies the derivative of the activation function to update
   * the input weights.
   *
